...
<%--
	...
	1. include	: 현재 페이지에서 다른 페이지 내용을 보여주는 것
		-지시어 include 	: 다른 페이지 소스 전체를 가져와서 현재 페이지에 한번에 처리
						  ...
						  (원본 페이지는 변경, 수정 되면 자동으로 다시 컴파일 되지만 
						   사본 페이지는 자동 컴파일 되지 않는다.)
						  <% include file="가져올 페이지" %>
		-액션태그 include 	: 다른 페이지의 소스가 아닌 결과 값을 현재 페이지에 포함 시킴
						  파라미터값을 전달 할 수 있다.
 ...
 --%>
	...
	<h2>include 액션 태그 사용</h2>
	<%--만약 이곳에서 한글이 깨지면 utf-8처리를 여기서 해줘야한다. 원래는 불러오는 곳의 페이지에서 하고 있다. --%>
